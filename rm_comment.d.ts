...
declare class Game_SelfSwitches {
     ...
     * 儲存遊戲中所有事件的自開關設定
     * 
     * 通常情況下直接查詢的結果只會是 `true` 或 `undefined` ，因為設定為 `false` 會直接刪除對應 `key` `value` 對。
     * 
     * 但直接賦值仍然可以設定為 `false`。
     */
     ...
     * 格式為 `[地圖編號, 事件編號, 自開關名稱]` 。
     * 
     * 在沒有其餘插件影響的前提下，`key` 會直接用於物件查詢，可以直接使用陣列的字串格式 `${地圖編號},${事件編號},${自開關名稱}` 替代。
     * @example 
     * const a1 = $gameSelfSwitch.value([1, 2, "A"]) // 取得一號地圖、二號事件的自開關 A
     * const a2 = $gameSelfSwitch.value(`1,2,A`)     // 在沒有插件影響的前提下，效果同上
     */
    value(key: [number, number, string] | string): boolean
    /**
     * 設定對應自開關狀態
     * 
     * 副作用：將要求 $gameMap 更新狀態。
     * @param key 
     * 格式為 `[地圖編號, 事件編號, 自開關名稱]` 。
     * 
     * 在沒有其餘插件影響的前提下，key 會直接用於物件查詢，可以直接使用陣列的字串格式 `${地圖編號},${事件編號},${自開關名稱}` 替代。
